...
const LoginPage: React.FC = () => {
  const [email, setEmail] = useState('juan.v@finca.co');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setError('');
    // Admin check
    if (email === 'admin@godcamp.co' && password === 'password') {
      login();
      return;
    }
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full max-w-md bg-white/80 backdrop-blur-md p-8 rounded-2xl shadow-2xl animate-scaleIn">
        <h1 className="text-3xl font-bold text-center text-brand-dark mb-2 font-heading">Iniciar Sesión</h1>
        <p className="text-center text-slate-700 mb-6">Accede a tu cuenta para continuar.</p>
        <form onSubmit={handleSubmit}>
         ...
         <p className="text-center mt-6 text-slate-800">
            ¿No tienes una cuenta? <Link to="/register" className="font-bold bg-gradient-to-r from-brand-green to-brand-gold bg-clip-text text-transparent hover:opacity-80 font-heading">Regístrate aquí</Link>
          </p>
